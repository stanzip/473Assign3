...
namespace Assign3
{
    public class Person : IComparable
    {
        ...
        public Person(string nameAgain, string occupationAgain, DateTime birthdayAgain)
        {
            firstName = nameAgain;
            occupation = occupationAgain;
            birthday = birthdayAgain;
        }

        public Person(uint id, string name, string occupation, DateTime birthday, uint residenceId)
        {
        ...
        {
            id = Convert.ToUInt32(input[0], 10);
            lastName = input[1];
            firstName = input[2];
            occupation = input[3];
        ...
        public bool addPersonToProperty(Property p)
        {
            uint resID = p.id;
            bool livesHere = false;

            if(!residenceIds.Contains(resID))
            {
                livesHere = true;
